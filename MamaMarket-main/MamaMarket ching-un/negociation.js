...
async function envoyerOffreAuServeur(idProduit, prixPropose) {
    // Note : Dans un vrai projet, l'idAcheteur vient de la session de connexion
    const idAcheteur = 1; 

    const data = {
        produit_id: idProduit,
        acheteur_id: idAcheteur,
    ...
    try {
        ...
        if (response.ok) {
            // Ici on utilise ta fonction de toast déjà présente dans ton code
            showToast('🤝', 'Offre envoyée !', `Ta proposition de ${prixPropose} FCFA a été transmise.`);
        } else {
            showToast('⚠️', 'Erreur', 'Impossible d\'envoyer l\'offre.');
        }
    } catch (error) {
...
function validerOffre() {
    const prix = document.getElementById('inputPrix').value; // Ton input de prix
    const pId = 1; // ID du produit (pour le test)

    fetch('http://localhost:3000/api/negocier', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
            produit_id: pId,
    ...
    .then(data => {
        ...
        alert("C'est bon ! L'offre est dans la base de données.");
    })
